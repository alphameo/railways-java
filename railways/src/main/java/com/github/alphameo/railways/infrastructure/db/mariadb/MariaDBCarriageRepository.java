...
public class MariaDBCarriageRepository implements CarriageRepository {
    ...
    public MariaDBCarriageRepository(Connection connection) {
        this.connection = connection;
    }

    @Override
    public void create(Carriage entity) {
        Id id = entity.getId();
    ...
    @Override
    public Optional<Carriage> findById(Id id) {
    ...
    private Carriage mapResultSetToCarriage(ResultSet rs) throws Exception {
        Id id = Id.fromString(rs.getString("id"));
        MachineNumber number = new MachineNumber(rs.getString("number"));
        CarriageContentType contentType = CarriageContentType.create(rs.getString("content_type"));
        Long capacity = rs.getLong("capacity");
        Carriage carriage = new Carriage(id, number);
